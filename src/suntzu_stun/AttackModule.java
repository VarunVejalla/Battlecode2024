...
//        RobotInfo[] friendliesThatCanAttackEnemyActionCOM = rc.senseNearbyRobots(enemyActionCOM, GameConstants.ATTACK_RADIUS_SQUARED, robot.myTeam);
    ...
    public void runSetup() throws GameActionException {
        ...
        if(wasSafeInPreviousRound && !heuristic.getSafe()){
            // if we were safe in the previous round and we're not safe now, try placing stun traps
            tryPlacingStunTrap();
        }
    ...
    public void tryPlacingStunTrap() throws GameActionException{
        ...
        for(Direction dir: Util.closeDirections(toEnemyCOM)){
            MapLocation potentialTrapLoc = robot.myLoc.add(dir);
            if (rc.canBuild(TrapType.STUN, potentialTrapLoc)) {
                rc.build(TrapType.STUN, potentialTrapLoc);
                return;
            }
        }
    }
